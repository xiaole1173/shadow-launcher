...
Rectangle {
    id: root
    implicitHeight: expandable ? col.implicitHeight + 24 : headerHeight + 12
    clip: true
    color: StyleTokens.bgPrimary
    border.color: StyleTokens.bgElevated; border.width: 1
    radius: StyleTokens.radiusLg
    Behavior on implicitHeight { NumberAnimation { duration: 350; easing.type: Easing.OutCubic } }
    ...
    MouseArea {
        ...
        onClicked: { root.expanded = !root.expanded
            if (root.expanded) root.expandable = true }
    }

    ColumnLayout {
        id: col
        width: parent.width - 28
        x: 14; y: expanded ? 14 : 8
        spacing: 10
