...
Window {
    ...
    // ── 官网页面直链（2026-09-24：v1.0.3 更新内容 / 启动器公告）──
    // 官网 VitePress 渲染，仅直链可访问（站点无导航入口、不进站内搜索）。
    // 路径含中文，此处直接写 percent-encoded，避免 QUrl 编码歧义。
    //   完整中文路径：/docs/notes/v1.0.3更新内容.html 、 /docs/notes/启动器公告.html
    readonly property string changelogUrl: "https://shadowlauncher.cn/docs/notes/v1.0.3%E6%9B%B4%E6%96%B0%E5%86%85%E5%AE%B9.html"
    ...
    // 打开官网 v1.0.3 更新内容页（左下角版本号 / 蓝色启动 toast 共用）
    function openChangelogPage() {
        if (backend) backend.logUiMsg("打开官网 v1.0.3 更新内容页")
    ...
    // ── v1.0.3 启动 toast（2026-09-24：沿用 v1.0.3 结构）──
    ...
    function maybeShowStartupToasts() {
        ...
        // 仅 v1.0.3 展示本次 toast；版本升级后自然停用（后续版本另行配置）
        if (backend.appVersion !== "v1.0.3") return
        // 本次是否属于"更新后首次启动"（新装/同版本重启为 false）
        var isUpdate = backend.consumeJustUpdatedFlag()
        var changelogDone = backend.readUiFlag("startup/changelog_v103_clicked")
        // 公告（黄 toast）标志沿用 v101：公告内容未更新，v1.0.1 已点过的用户 v1.0.3 不重复弹
        var announcementDone = backend.readUiFlag("startup/announcement_v101_clicked")
        // 蓝色 toast（更新内容）：仅更新用户弹出
        if (isUpdate && !changelogDone) {
            toastManager.showAction("您已成功更新至v1.0.3，点击查看v1.0.3更新内容。", function() {
                appWindow.openChangelogPage()
                if (backend) backend.writeUiFlag("startup/changelog_v103_clicked", true)
            }, "info")
        }
    ...
    Component.onCompleted: {
        ...
        // v1.0.3 启动 toast（延迟触发，避免与窗口初始化竞争）
    ...
    // v1.0.3 启动 toast（2026-09-24：延迟触发，待窗口与后端就绪）
    Timer {
        ...
        RowLayout {
            ...
            Rectangle {
                ...
                ColumnLayout {
                    ...
                    // 版本号（2026-09-06：可点击 → 打开官网 v1.0.3 更新内容页）
                    Item {
