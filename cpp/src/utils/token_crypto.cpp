...
QByteArray TokenCrypto::loadOrCreateKey()
{
    ...
    if (file.open(QIODevice::ReadOnly)) {
        ...
        if (!blob.isEmpty()) {
            ...
            if (key.size() == 32) {
                qCInfo(logAccount) << "TokenCrypto: loaded existing key";
                return key;
            }
            qCWarning(logAccount) << "TokenCrypto: existing key invalid, regenerating";
        }
    }
    ...
    if (blob.isEmpty()) {
        qCCritical(logAccount) << "TokenCrypto: failed to protect key!";
        return {};
    }
    ...
    if (file.open(QIODevice::WriteOnly | QIODevice::Truncate)) {
        ...
        qCInfo(logAccount) << "TokenCrypto: new key generated and protected";
    } else {
        qCCritical(logAccount) << "TokenCrypto: failed to write key file!";
        return {};
    }
...
QByteArray TokenCrypto::protectKey(const QByteArray &key)
{
    ...
    if (!CryptProtectData(&inBlob,
                          ...
                          &outBlob)) {
        qCWarning(logAccount) << "TokenCrypto: CryptProtectData failed:" << GetLastError();
        return {};
    }
...
QByteArray TokenCrypto::unprotectKey(const QByteArray &blob)
{
    ...
    if (!CryptUnprotectData(&inBlob,
                            ...
                            0,
                            &outBlob)) {
        qCWarning(logAccount) << "TokenCrypto: CryptUnprotectData failed:" << GetLastError();
        return {};
    }
...
{
    if (key.size() != 32) {
        qCWarning(logAccount) << "AES-GCM encrypt: key must be 32 bytes";
        return {};
    }
    ...
    if (!BCRYPT_SUCCESS(status)) {
        qCWarning(logAccount) << "BCryptOpenAlgorithmProvider failed:" << status;
        return {};
    }
    ...
    if (!BCRYPT_SUCCESS(status)) {
        qCWarning(logAccount) << "BCryptSetProperty GCM failed:" << status;
    ...
    if (!BCRYPT_SUCCESS(status)) {
        qCWarning(logAccount) << "BCryptGenerateSymmetricKey failed:" << status;
        BCryptCloseAlgorithmProvider(hAlg, 0);
        return {};
    }
    ...
    if (!BCRYPT_SUCCESS(status)) {
        qCWarning(logAccount) << "BCryptEncrypt failed:" << status;
        return {};
    }
...
static QByteArray aesGcmDecrypt(const QByteArray &key, const QByteArray &combined)
{
    ...
    if (!BCRYPT_SUCCESS(status)) {
        qCWarning(logAccount) << "BCryptDecrypt failed (auth tag mismatch or corrupt):" << status;
        return {};
    }
