...
QString AccountBackend::renderHead3D(const QString& skinPath)
{
    ...
    const float yr = 25.0f*DEG, pr = -10.0f*DEG;
    const float cy=cosf(yr),sy=sinf(yr), cp=cosf(pr),sp=sinf(pr);
    // Scale: head (8u) → ~48px centered in 64px canvas, leaving hat margin
    const float SC = (float)SZ / 10.5f;  // head ~49px, hat ~52px, margin ~6px
    ...
    auto emitQuad = [&](const float cx[4],const float cy[4],const float cz[4],
                         int u0,int v0,int u1,int v1, bool isHt)
    {
        ...
        if (tc+2>24) return;
        tri[tc]={sx[0],sy[0],sx[1],sy[1],sx[2],sy[2], US[0],VS[0],US[1],VS[1],US[2],VS[2], (sd[0]+sd[1]+sd[2])/3.0f, isHt}; tc++;
        tri[tc]={sx[0],sy[0],sx[2],sy[2],sx[3],sy[3], US[0],VS[0],US[2],VS[2],US[3],VS[3], (sd[0]+sd[2]+sd[3])/3.0f, isHt}; tc++;
    };
