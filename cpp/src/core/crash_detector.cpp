...
{
    qCDebug(logLaunch) << "[CRASH] scanning in" << gameDir;
    ...
    // ── Check crash-reports/ folder for Minecraft crash reports ──
    {
        ...
        if (crashDir.exists()) {
            ...
            if (!entries.isEmpty()) {
                ...
                qCDebug(logLaunch) << "[CRASH] found minecraft crash report:" << newest.fileName();
    ...
    // ── Check game dir root for hs_err_pid*.log (JVM crash) ──
    {
        ...
        if (!entries.isEmpty()) {
            const QFileInfo& newest = entries.first();
            qCDebug(logLaunch) << "[CRASH] found JVM crash report:" << newest.fileName();
            CrashReport r = parseJvmCrash(newest.absoluteFilePath());
            if (r.isValid) return r;
        }
    }

    qCDebug(logLaunch) << "[CRASH] no crash reports found";
    return CrashReport{};
}
...
CrashReport CrashDetector::parseJvmCrash(const QString& filePath)
{
    ...
    qCDebug(logLaunch) << "[CRASH] parsed JVM crash:" << r.reason;
    return r;
}
...
CrashReport CrashDetector::parseMinecraftCrash(const QString& filePath)
{
 ...
 QString descLine = line.mid(3).trimmed();
    ...
    qCDebug(logLaunch) << "[CRASH] parsed minecraft crash:" << r.reason;
    return r;
}
